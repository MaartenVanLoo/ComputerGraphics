...
MRay::Vec3::Vec3(double x, double y, double z) {
    assert(std::abs(x) < FLT_MAX);
    assert(std::abs(y) < FLT_MAX);
    assert(std::abs(z) < FLT_MAX);
#if SET_DATA
    // this->data = _mm256_set_pd (w,z,y,x); //double
    this->data = _mm_set_ps (0,(float)z,(float)y,(float)x); //float
#else
    this->data[0] = x;
    this->data[1] = y;
    this->data[2] = z;
...
#if SET_DATA
    //double:
    //this->data = _mm256_set_pd(w, vec3.get<2>(),vec3.get<1>(),vec3.get<0>()); //double

    //float:
    this->data = _mm_set_ps(0, (float)vec.get<1>(),(float)vec.get<0>(),x); //float
#else
    this->data[0] = x;
...
#if SSE_AVX_EXTENSIONS
#if SET_DATA
    const __m128 negate_ps = _mm_set_ps(-1.0f, -1.0f, -1.0f, -1.0f); //float
...
Vec3 &MRay::Vec3::operator*=(const double rhs) {
    assert(std::abs(rhs) < FLT_MAX);
#if SSE_AVX_EXTENSIONS
#if SET_DATA
    __m128 rhs_ps = _mm_set_ps((float)rhs, (float)rhs,(float)rhs,(float)rhs); //float
    ...
    __m128 lhs_ps = _mm_set_ps(0, this->data[2], this->data[1], this->data[0]); //float
    __m128 rhs_ps = _mm_set_ps((float)rhs, (float)rhs, (float)rhs, (float)rhs); //float
    rhs_ps = _mm_mul_ps (lhs_ps , rhs_ps);
    _mm_store_ps (this->data, rhs_ps);
#endif
#else
    this->data[0]*=(float)rhs;
    this->data[1]*=(float)rhs;
    this->data[2]*=(float)rhs;
#endif
    return *this;
}
Vec3 &MRay::Vec3::operator*=(const Vec3& rhs) {
...
#if SSE_AVX_EXTENSIONS
#if SET_DATA
    const __m128 rhs_ps = _mm_set_ps((float)rhs, (float)rhs, (float)rhs, (float)rhs); //float
    this->data = _mm_div_ps(this->data,rhs_ps); //float
#else
    __m128 lhs_ps = _mm_set_ps(this->data[3], this->data[2], this->data[1], this->data[0]); //float
    __m128 rhs_ps = _mm_set_ps((float)rhs,(float)rhs,(float)rhs,(float)rhs);
    // __m256d rhs_pd = _mm256_set_pd (rhs,rhs,rhs,rhs); //double
...
Vec3 MRay::operator*(const double &lhs, Vec3 rhs) {
    assert(std::abs(lhs) < FLT_MAX);
    rhs*=lhs;
    return rhs;
}
Vec3 MRay::operator*(Vec3 lhs, const double &rhs) {
    assert(std::abs(rhs) < FLT_MAX);
    lhs*=rhs;
    return lhs;
}
...
#if SSE_AVX_EXTENSIONS
#if SET_DATA
    const __m128 lhs_ps = _mm_set_ps((float)lhs, (float)lhs, (float)lhs, (float)lhs); //float
...
#pragma region Vec4
MRay::Vec4::Vec4(double x, double y, double z, double w) {
    assert(std::abs(x) < FLT_MAX);
    assert(std::abs(y) < FLT_MAX);
    assert(std::abs(z) < FLT_MAX);
    assert(std::abs(w) < FLT_MAX);
#if SET_DATA
    // this->data = _mm256_set_pd (w,z,y,x); //double
    this->data = _mm_set_ps ((float)w,(float)z,(float)y,(float)x); //float
#else
    this->data[0] = (float)x;
    this->data[1] = (float)y;
    this->data[2] = (float)z;
...
MRay::Vec4::Vec4(const Vec3 &vec3, double w) {
    assert(std::abs(w) < FLT_MAX);
#if SET_DATA
    //double:
    //this->data = _mm256_set_pd(w, vec3.get<2>(),vec3.get<1>(),vec3.get<0>()); //double
    ...
    //float:
    this->data = _mm_set_ps((float)w, vec3.get<2>(),vec3.get<1>(),vec3.get<0>()); //float
#else
...
Vec4 &MRay::Vec4::operator*=(const double rhs) {
    assert(std::abs(rhs) < FLT_MAX);
#if SSE_AVX_EXTENSIONS
#if SET_DATA
    const __m128 rhs_ps = _mm_set_ps((float)rhs, (float)rhs, (float)rhs, (float)rhs); //float
    ...
    // __m256d rhs_pd = _mm256_set_pd (rhs,rhs,rhs,rhs); //double
    // this->data = _mm256_mul_pd(res.data, rhs_pd);
    const __m128 lhs_ps = _mm_set_ps(this->data[3], this->data[2], this->data[1], this->data[0]); //float
    __m128 rhs_ps = _mm_set_ps((float)rhs, (float)rhs, (float)rhs, (float)rhs); //float
    rhs_ps = _mm_mul_ps (lhs_ps , rhs_ps);
    _mm_store_ps (this->data, rhs_ps);
#endif
...
#endif
#else
    rhs.data[0] = (float)lhs/rhs.data[0];
    rhs.data[1] = (float)lhs/rhs.data[1];
    rhs.data[2] = (float)lhs/rhs.data[2];
    rhs.data[3] = (float)lhs/rhs.data[3];
#endif
    return rhs;
}
Vec4 MRay::operator/(Vec4 lhs, const Vec4 &rhs) {
...
Vec4 Vec4::max(Vec4 lhs, const Vec4 &rhs) {
    ...
    return lhs;
#else
    const __m128 lhs_ps = _mm_set_ps(lhs.data[3], lhs.data[2], lhs.data[1], lhs.data[0]); //float
    __m128 rhs_ps = _mm_set_ps(rhs.data[3], rhs.data[2], rhs.data[1], rhs.data[0]); //float
    rhs_ps = _mm_max_ps(lhs_ps, rhs_ps);
...
Vec4 Vec4::min(Vec4 lhs, const Vec4 &rhs) {
    ...
    const __m128 lhs_ps = _mm_set_ps(lhs.data[3], lhs.data[2], lhs.data[1], lhs.data[0]); //float
    __m128 rhs_ps = _mm_set_ps(rhs.data[3], rhs.data[2], rhs.data[1], rhs.data[0]); //float
    rhs_ps = _mm_min_ps(lhs_ps, rhs_ps);
    _mm_store_ps (lhs.data, rhs_ps);
    return lhs;
#endif
#else
